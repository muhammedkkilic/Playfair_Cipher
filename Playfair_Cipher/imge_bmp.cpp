...
BYTE* ConvertBMPToIntensity(BYTE* Buffer, int width, int height)
{
	...
	//MessageBox::Show(Convert::ToString(psw));
	// create new buffer
	BYTE* newbuf = new BYTE[width*height];    //  3 Bytes
...
BYTE* ConvertIntensityToBMP(BYTE* Buffer, int width, int height, long* newsize)
{
	...
	*newsize = height * psw;
	...
	BYTE* newbuf = new BYTE[*newsize];    //1 Bytes
...
BYTE* ConvertBMPToRGBdizi(BYTE* Buffer, int width, int height)
{
	// first make sure the parameters are valid
	if ((NULL == Buffer) || (width == 0) || (height == 0))
		return NULL;
	...
	BYTE* sonuc = new BYTE[width * height * 3]{};
	...
	for (int row = 0; row < height; row++)
	{
		...
		{
			bufpos = (height - row - 1) * psw + column * 3;
			sonuc[(row * 3) * width + (column * 3)] = Buffer[bufpos];
			sonuc[(row * 3) * width + (column * 3 + 1)] = Buffer[bufpos + 1];
			sonuc[(row * 3) * width + (column * 3 + 2)] = Buffer[bufpos + 2];
		}
	}
	return sonuc;
}
